...
import com.example.congviecktra.MainActivity;
...
public class UpdateDeleteActivity extends AppCompatActivity {
    private EditText edName, edAuthor, edPublishDate;
    private CheckBox edPrice;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.v("kiemtra",congviec.toString());
        edName.setText(congviec.getTencongviec());
        edAuthor.setText(congviec.getNoidungcongviec());
        edPublishDate.setText(congviec.getNgayhoanthanh());
        Log.v("kiemtra2",congviec.getTinhtrang());
        if(congviec.getCongtac().equals("Lam chung")){
            edPrice.setChecked(true);
        }
        else edPrice.setChecked(false);
        for(int i=0;i<spPublisher.getCount();i++){
            ...
            if(congviec.getTinhtrang().equals(spPublisher.getItemAtPosition(i))){
        ...
        btnUpdate.setOnClickListener(view -> {
            ...
            String congtac ="";
            if (edPrice.isChecked()){
                congtac = "Lam chung";
            }
            else congtac = "lam 1 mình";

            CongViec congViecToUpdate = new CongViec(congviec.getId(),name,author,publishDate,publisher,congtac);

            SQLiteHelper sqLiteHelper = new SQLiteHelper(UpdateDeleteActivity.this);
            sqLiteHelper.updateCongViec(congViecToUpdate);
